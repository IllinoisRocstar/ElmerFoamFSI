...
set InputDir = SimpleStatic_4
set Outputs = ./fluid/probe_proc_0_nde_0.dat
set OutputsCheck = ./fluid/ref_4.dat
set TestName = SimpleStatic_4:Works
...
./AllrunParDrvSetup
cd ./fluid
cat <<EOF > ./regStatic_parallel_test_batch.csh
#!/bin/tcsh
#
#PBS -V
#PBS -l nodes=1:ppn=8
...
#PBS -j oe
#PBS -o regStatic_parallel_test_batch_output
#PBS -A MPINFRA

cd \${PBS_O_WORKDIR}
mpirun -np 4 -machinefile \${PBS_NODEFILE} ${BINDIR}/elmerfoamfsiPar -v 2 test.config
echo "1" >> regStaticFinished
exit 0
EOF
qsub regStatic_parallel_test_batch.csh
@ i = 1
while($i <= 720)
    @ i += 1
    if( -e regStaticFinished ) then
        @ i += 721;
    else
        sleep 10;
    endif
