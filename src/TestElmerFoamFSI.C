...
namespace ElmerFoamFSI{
  ...
  int Test(int argc,char *argv[])
  {
    ...
    if(!comline.GetOption("help").empty()){
      ...
      if(verblevel > 2)
        std::cout << "ElmerFoamFSI::Test: Exiting test function (success)" << std::endl;
      return(0);
    }
    if(clerr){
      ...
      if(verblevel > 2)
        std::cout << "ElmerFoamFSI::Test: Exiting test function (fail)" << std::endl;
      return(1);
    }
   ...
   // The following block parses and sets the verbosity level
    ...
    if(!OutFileName.empty()){
      ...
      if(!Ouf){
        std::cout << "ElmerFoamFSI::Test> Error: Could not open output file, " 
    ...
    if(verblevel > 2)
      std::cout << "ElmerFoamFSI::Test: Entering test function" << std::endl;
    ...
    // run the listed tests.
      ...
      if(!ListInf){
        std::cout << "ElmerFoamFSI::Test> Error: Could not open list of tests in file " 
    ...
    *Out << results << std::endl;
    ...
    if(verblevel > 2)
      *Out << "ElmerFoamFSI::Test: Exiting test function (success)" << std::endl;
...
{
  return(ElmerFoamFSI::Test(argc,argv));
}
