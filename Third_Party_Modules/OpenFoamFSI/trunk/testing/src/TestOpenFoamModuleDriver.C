...
namespace OpenFoamModuleDriver{
  ...
  int Test(int argc,char *argv[])
  {
    ...
    if(!comline.GetOption("help").empty()){
      ...
      if(verblevel > 2)
        std::cout << "OpenFoamModuleDriver::Test: Exiting test function (success)" << std::endl;
      return(0);
    }
    if(clerr){
      ...
      if(verblevel > 2)
        std::cout << "OpenFoamModuleDriver::Test: Exiting test function (fail)" << std::endl;
      return(1);
    }
    ...
    if(!OutFileName.empty()){
      ...
      if(!Ouf){
        std::cout << "OpenFoamModuleDriver::Test> Error: Could not open output file, " 
    ...
    if(verblevel > 2)
      std::cout << "OpenFoamModuleDriver::Test: Entering test function" << std::endl;
    
    // Make an instance of the OpenFoamModuleDriver testing object, OpenFoamModuleDriver::TestingObject
    OpenFoamModuleDriver::TestingObject<OpenFoamModuleDriver::TestResults> test;
    ...
    // run the listed tests.
      ...
      if(!ListInf){
        std::cout << "OpenFoamModuleDriver::Test> Error: Could not open list of tests in file " 
...
{
  return(OpenFoamModuleDriver::Test(argc,argv));
}
